...
#s = "aaJJJJJJJJJJJJJJJabbcvbNNNbNNNbCDEfGHIkklmmnoLLLmLLl"
...
#            print(s[ch-3:ch])
#            print(s[ch+1:ch+4])
#            if s[ch-3:ch] in uc and s[ch+1:ch+4] in uc:
#                print(ch)
#    except:
#        continue
...
for t in s:
    if len(result) == 7 and found_lc == True:
        if t in lc:
            finalists.append(result)
            result = ""

    if t in lc and len(result) == 0:
        preceding_lc = True
        continue

    if preceding_lc == True:
        if t in uc:
            result += t
        elif len(result) == 3:
            result += t
...
# above is without regex, middle characters are 'linkedlist'

# with regex

# import re

# re.findall("[^A-Z]+[A-Z]{3}[a-z][A-Z]{3}[^A-Z]", s)
